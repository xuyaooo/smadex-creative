...
def test_no_changepoint_on_constant_series():
    ...
    out = fatigue_changepoint(series, hazard_lambda=50.0, threshold=0.4)
...
def test_clear_changepoint_fires_under_low_threshold():
    ...
    out = fatigue_changepoint(series, hazard_lambda=10.0, threshold=0.05)
    ...
    out_high = fatigue_changepoint(series, hazard_lambda=10.0, threshold=0.5)
...
def test_short_series_does_not_crash():
    ...
    out = fatigue_changepoint(np.array([0.1, 0.1, 0.1]), threshold=0.4)
    assert out["has_changepoint"] is False
    assert out["changepoint_day"] == -1
